...
static struct user *create_user(char *name, int id)
{
    ...
    if (name != NULL)
    {
        space->name = malloc(strlen(name) + 1);
        if (space->name == NULL)
        {
            free(space);
            return NULL;
...
static int user_equal(const struct user *user1, const struct user *user2)
{
    ...
    else if (user1->name == NULL && user2->name == NULL && (user1->id == user2->id))
    {
        return 1;
    }
...
int replace_name_at_index(struct linked_list *list, int index, char *newName)
{
    ...
    char * t= realloc(temp->data->name, strlen(newName) + 1);
    if (t == NULL)
    {
        return FAILURE;
    }
