...
int draw_hollow_box(struct ascii_image *image, int row, int col, int height, int width, char c) {
    ...
    for (int i = row; i < height+row-1; i++){
        set_character(image, i, col, c);
        set_character(image, i, col+width-1, c);
    }
    for (int i = col; i < width+col; i++){
        set_character(image, row, i, c);
        set_character(image, row+height-1, i, c);
    }
...
struct ascii_image *create_image(int height, int width, char *name) {
    ...
    // These are just to turn off compiler errors initially
    // Please delete once you have implemented the function
    struct ascii_image * i = malloc(sizeof(struct ascii_image));
    if (i == NULL || name == NULL||width < 1||height<1){
        free(i);
;       return NULL; 
    }
    i->height = height;
    i->width = width;
    i->data = malloc(height*width);
    ...
    return i;    // Don't forget to replace this!
}
...
int add_extension(struct ascii_image *image, char *extension) {
    ...
    char* temp = realloc(image->name, length + strlen(extension) + 1);
    if (!temp){
        return FAILURE;
    }
    
    image->name = temp;
    strcpy(image->name + length, extension);
    return SUCCESS;
}
